...
# if IN_GITHUB_ACTIONS is set, we are running in a GitHub Actions environment
# and the CMAKE_PREFIX_PATH should be /usr/local/
# otherwise the value is ../../LOCAL_INSTALL_DIR
if [ -z "$IN_GITHUB_ACTIONS" ]
then
      CMAKE_PREFIX_PATH="../../$LOCAL_INSTALL_DIR/"
else
...
mkdir build_man
cd build_man
cmake -DCMAKE_INSTALL_PREFIX=$CMAKE_PREFIX_PATH  -DPYBIND11_TEST=OFF ..
make install -j4
cd ../../
...
mkdir build_man
cd build_man
cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_INSTALL_PREFIX=$CMAKE_PREFIX_PATH ..  
cmake --build . 
...
mkdir build_man
cd build_man
cmake -DCMAKE_INSTALL_PREFIX=$CMAKE_PREFIX_PATH -DENABLE_STATIC=FALSE -DCMAKE_BUILD_TYPE=Release ..
